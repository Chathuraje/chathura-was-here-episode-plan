...
Status: **draft plan, awaiting Chathura's review**
Prepared: 2026-09-18
...
This document is the single plan for getting from where the project is now to a full production screenplay for every development film (Episodes 2–99). The earlier documents set the rules. This one sets the **order of work**, what each step produces, and what Chathura decides at each gate.
...
| Rules & data model | Written (docs 02–05), **not implemented** |
| Concept → idea pilot | 3 concepts (C020, C032, C084), 2 draft ideas, 1 brief preview, all awaiting review |
| Dashboard | Read-only source/concept browser. No ideas, groups, objects, locations, releases, briefs or screenplays |
| Working tree | 70 concept extract files show as modified, but the change is **line endings only** (CRLF/LF from the shared drive). No text changed |
...
RELEASE ORDER (what the audience sees)
  Ep 1 "The Beginning" ─▶ location-based series … (Ep 2–99, any series length) ─▶ Ep 100 "The Way Back"
...
- **100 films = 2 framing films (Ep 1, Ep 100) + 98 development films (Ep 2–99).**
...
- **Release order** ignores chronology and is organised around locations: multi-episode series with no fixed length.
...
### Step 6: Release plan *(location-first, chronology ignored)*
...
- Ep 1 fixed at the start and Ep 100 fixed at the end. Positions 2–99 are assigned here for the first time.
...
- Film counts are drafts; they are finalised in Step 4. The concept "roles" in each group record are English working glosses, which Step 2 verifies against the sources.
...
| D6: Release planner | Step 6 | Location-based series, public numbering 2–99, Ep 1/Ep 100 pinned, chronology-vs-release comparison |
...
- **Dashboard:** `/chronology` (slate board), `/episodes/[id]` (episode brief plus Chathura's location picker), `/release` (location-based series proposal) and the home pipeline. These are not yet build-tested; that waits for Chathura's go-ahead.
- **Dashboard build-tested (2026-09-19)** on an isolated copy: every page loads, and the location select, record, clear and release-series flow works end to end. `/locations` gives an overview of all 98 films for Step 5.
- **Screenplay system ready (Step 8):** `docs/planning/prompts/screenplay-brief.md` (modelled on the Episode 1 guide), `development/screenplays/SPV-NNNN.json` records, `/screenplays` pipeline and version pages. Each episode brief names the next stage. The validator blocks any screenplay without Chathura's selected location and enforces treatment → outline → production.
- **Viewer lessons added (2026-09-19):** every episode and group now has a `lesson`: a plain-English line plus a Sinhala line, the Abhidhamma teaching with exact citations, how the film shows it, and a caution. Lessons avoid every digest-flagged uncertainty. They appear on episode pages, group pages, the `/learning` path and in episode briefs, and the validator checks them. The writing brief is `docs/planning/prompts/lesson-brief.md`.
- **Now with Chathura:** choose locations at `/locations`. Each choice unlocks that film's treatment → outline → screenplay, and the release planner (`/release`) forms series from the chosen regions.
