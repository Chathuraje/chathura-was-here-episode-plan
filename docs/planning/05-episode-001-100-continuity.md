# Episode 001/100 continuity foundation

This document separates repository presence, external creative references, intended continuity, and verified footage/event evidence. It does not convert screenplay plans into claims about what was filmed or what occurred.

## Historical repository finding

At the original Step 1 audit, no Episode 1, Episode 99, or Episode 100 screenplay was present in this repository. No channel instruction document, footage log, transcript, edit decision list, or shared-scene register was present either. That repository finding remains true.

The original audit therefore could not verify scene text, dialogue, footage IDs, timecodes, technical edit boundaries, or participant actions from repository files. The dated external-reference follow-up below adds sources without rewriting that historical finding.

## External references — retrieved 2026-09-18

All three identified Google Docs were accessible through the authorized Google Drive connection and were read on 2026-09-18:

| Reference | Repository status | Retrieval status | Local planning snapshot |
|---|---|---|---|
| [Channel Details](https://docs.google.com/document/d/1unE3metwZwMDAjgNPPtCJ97_BDV5SeHL-oF0itFijHU) | Not present in repository | Identified and retrieved | `docs/planning/references/channel-details-2026-09-18.md` |
| [The Beginning](https://docs.google.com/document/d/10fWaF9Vl_4_IzohQfDQLiXu2Y0nI2GVVFPHAy3B2KHs) | Not present in repository | Identified and retrieved | `docs/planning/references/the-beginning-2026-09-18.md` |
| [The Way Back](https://docs.google.com/document/d/1IjQCv2l4ECAc_MLG18a71a-flpB3gEFsdpvNxCpxVmY) | Not present in repository | Identified and retrieved | `docs/planning/references/the-way-back-2026-09-18.md` |

These documents corroborate the intended framing relationships below. They are creative/channel references, not footage logs or independent evidence that planned events happened. The screenplays also contain planned or placeholder material that must not be presented as authentic dialogue or verified action.

## Intended creative continuity

The current intended relationships are:

1. **Episode 1 opens in the crowded location.** It uses the crowded-location footage and cuts away to the road toward Meemure.
2. **Episode 100 opens with that same crowded footage.** It reaches Episode 1’s planned cut point but continues beyond it instead of cutting away.
3. **Episode 100 Part A fills the first missing span.** It continues through Chathura’s room, reveals all ten objects together in one frame before packing them, then covers departure and travel toward Hunnasgiriya/Meemure.
4. **Part A rejoins Episode 1.** It reaches the exact planned road-footage point where Episode 1 originally resumed.
5. **Episode 100 revisits selected Episode 1 material.** The revisit moves through selected road, monastery, and related images until it reaches Chathura’s forest disappearance.
6. **Episode 100 Part B fills the second missing span.** It continues beyond the forest ending.
7. **Episode 99 also uses the crowded footage at its ending.** Confirmed by Chathura on 2026-09-18: Episode 99 is the chronologically last development film (end of GRP-10). Footage and timecodes remain unverified.

The framing shorthand remains:

```text
Episode 100 Part A
  same crowd, extended beyond Episode 1 cut
  → room → all ten objects together in one frame → packing → departure
  → Hunnasgiriya / road toward Meemure
  → exact planned road point where Episode 1 resumed
      ↓
Episode 1 material revisited inside Episode 100
  selected road / monastery material
  → forest disappearance
      ↓
Episode 100 Part B
  continuation beyond the forest ending
```

This describes planned story and edit relationships. Actual footage IDs, source timecodes, edit in/out timecodes, verified events, and exact captured continuity remain unknown until supplied.

## Shared scene/event register

The labels below are descriptive planning labels, not implemented IDs.

| Stable scene/event identity | Chronological relationship | Episode appearances | Status and unknowns |
|---|---|---|---|
| Crowded-location event | Before room/packing and the journey toward Meemure in intended story chronology | Episode 1 opening cuts away; Episode 100 opening continues beyond that cut; Episode 99 ending is planned to share it | Episode 1/100 relationship is intended and supported by external references; Episode 99 appearance remains provisional. Footage ID and all timecodes unknown |
| Road-resumption event | After room, packing, departure, and travel; before the monastery sequence | Episode 1 resumes here; Episode 100 Part A reaches the same planned point | Matching composition is specified; actual common footage, frame identity, footage ID, and timecodes remain unverified |
| Selected Episode 1 revisit | After the road-resumption point and before the forest disappearance | Original Episode 1 sequence; selected material appears again within Episode 100 | Selection and intended meaning are planned; final shot list and edit boundaries unknown |
| Forest-disappearance event | After the monastery experience; before beyond-forest continuation | Episode 1 ends after the disappearance/empty forest; Episode 100 reaches it and continues in Part B | Intended relationship is supported by both screenplays; footage ID, handoff timecode, and verified action remain unknown |

Stable scene/event identity must remain independent of any episode label. Chronology is recorded as explicit before/after relationships. Every episode use is a separate appearance with its own edit boundaries, sound/narration treatment, and intended meaning. A single shared scene may therefore appear in Episodes 1, 99, and 100 without being duplicated or sorted by those episode numbers.

The same footage may gain a different meaning in Episode 100 because the audience has accumulated the intervening films. Planned continuity and verified footage/event evidence must remain separate at every stage.

## Object state

Current intended chronology says **all ten objects have already been acquired by Episode 1**. Record this as planned ownership state for Episode 1 and all later chronological scenes unless an intended or real possession-change event says otherwise.

Keep four questions separate:

| Dimension | Current state |
|---|---|
| Planned ownership | All ten objects are already acquired in the intended Episode 1 chronology |
| Verified acquisition | Unknown until real acquisition events and evidence are supplied |
| Possession changes | None verified; record transfers, loss, disposal, or other changes separately if reality supplies them |
| Shot visibility | Independent of ownership; an object outside the frame remains owned. Ordinary Episode 1 and Episode 100 shots need not show every object. The specific Episode 100 room reveal is the planned exception: all ten objects together in one frame before packing |

The Episode 100 room reveal must establish the collection with all ten objects together in one frame before packing. That one-frame composition is planned creative intent; no footage, timecode, or captured event currently verifies it. It does not turn visibility into the ownership test for other shots. Physical identity, images, acquisition date/place, supporting evidence, exact packing coverage, and later possession changes remain unresolved.

## Planned boundaries versus technical boundaries

The external references now establish three planned narrative boundaries:

- Episode 1’s crowd-to-road cut versus Episode 100’s continuation beyond that point;
- Episode 100 Part A’s arrival at the planned road point where Episode 1 resumed;
- Episode 1’s forest ending versus Episode 100 Part B’s continuation beyond it.

They do not provide usable footage IDs or technical edit timecodes. The words “same,” “exact moment,” “exact frame,” or “continues” express intended matching relationships; an editor must still verify the underlying footage and record source/sequence in/out points.

## Future footage and evidence requirements

- identify the crowded-location source footage and create separate Episode 1, Episode 99, and Episode 100 appearance records;
- verify the Episode 1 crowd cut and Episode 100 continuation with source and sequence timecodes;
- identify room, packing, departure, and journey footage; specifically verify whether the planned room-reveal frame contains all ten objects together before packing, while making no such visibility assumption for ordinary shots;
- verify the road match where Part A rejoins Episode 1;
- identify every selected Episode 1 shot reused or revisited in Episode 100;
- verify the forest disappearance and beyond-forest handoff;
- preserve natural sound, room tone, narration, and sound-bridge decisions per appearance;
- link transcripts, releases/permissions, and verified event notes where applicable;
- replace placeholder dialogue or actions with authentic recorded material, or remove them.

“Toward Hunnasgiriya/Meemure” is a continuity requirement, not automatic approval of a selected production location. Chathura retains exclusive location selection, and the disclosure timing of either name remains a separate decision.

## Continuity protections

- Never fabricate crowd behavior, monastery events, participant dialogue, the route actually taken, object acquisition, or what occurs beyond the forest.
- Retain Episode 99’s crowd ending as provisional until newer material or evidence supersedes it.
- Keep planned ownership separate from verified acquisition and possession history.
- Keep ownership separate from shot visibility, while retaining the explicit planned Episode 100 room-reveal exception.
- Preserve production screenplay versions separately from post-filming revisions grounded in captured footage.
- Reconcile this document against footage logs, transcripts, and edit decisions when they become available.
